...
export WORKER_ID=aia2025-s3
export RUN_MODE=PRODUCTION
export NUM_SHARDS=4
export SHARD_INDEX=3
unset MAX_BLOCKS_THIS_RUN

LOG="$HOME/solar_flare_aia/logs/aia2025_shard3_console.log"

echo "===== 2025 SHARD 3 STARTED: $(date -u) =====" | tee -a "$LOG"
...
jupyter nbconvert \
  ...
  --output "$HOME/solar_flare_aia/logs/aia2025_shard3_executed.ipynb" \
  2>&1 | tee -a "$LOG"

status=${PIPESTATUS[0]}
echo "===== 2025 SHARD 3 ENDED: $(date -u), STATUS=$status =====" | tee -a "$LOG"
exit "$status"
